...
#FAÇA AQUI OS EXERCÍCIOS alter
select * from estado;
select * from cidade;

#Exercício 1
SELECT cliente.cod_cli, cliente.nome_cli, cliente.cpf_cli, 
sexo.nome_sex,
endereco.rua_end, endereco.numero_end, endereco.bairro_end,
cidade.nome_cid,
estado.nome_est
FROM Cliente JOIN Sexo, Endereco, Cidade, Estado WHERE 
...
#Exercício 2
SELECT funcionario.cod_func, funcionario.nome_func, funcionario.cpf_func, funcionario.função_func, 
sexo.nome_sex,
endereco.rua_end, endereco.numero_end, endereco.bairro_end,
cidade.nome_cid,
estado.nome_est,
departamento.nome_dep
FROM funcionario JOIN Sexo, Endereco, Cidade, Estado, Departamento
WHERE funcionario.cod_sex_fk = sexo.cod_sex AND 
funcionario.cod_end_fk = endereco.cod_end AND
endereco.cod_cid_fk = cidade.cod_cid AND
cidade.cod_est_fk = estado.cod_est;

#Tarefa 3
-- pegue todos os dados de cliente quando existir um id desse cliente na tabela de carros
select cliente.nome_cli, carro.modelo_car FROM Cliente JOIN carro WHERE Cliente.cod_cli = carro.cod_cli_fk;

#Tarefa 4
SELECT Compra.cod_comp, Compra.data_comp, Compra.valortotal_comp, 
fornecedor.razãosocial_forn,
produto.descrição_prod, produto.valor_prod, 
itens_compra.quant_itenc FROM Compra JOIN Fornecedor, itens_compra, produto
WHERE compra.cod_forn_fk = fornecedor.cod_forn AND
itens_compra.cod_comp_fk = compra.cod_comp AND
itens_compra.cod_prod_fk = produto.cod_prod;

#Tarefa 5
SELECT venda.cod_vend, venda.data_vend, venda.valortotal_vend,
cliente.nome_cli, 
produto.descrição_prod, produto.valor_prod,
itens_venda.quant_itenv FROM Venda JOIN Produto, Cliente, itens_venda 
WHERE Venda.cod_cli_fk = cliente.cod_cli AND
Venda.cod_vend = itens_venda.cod_vend_fk AND
itens_venda.cod_prod_fk = produto.cod_prod;

#Tarefa 6
select pagamentos.cod_pag, pagamentos.data_pag, pagamentos.valor_pag, pagamentos.formapagamento_pag, 
caixa.cod_cai, caixa.dataabertura_cai, caixa.datafechamento_cai,
funcionario.nome_func, 
despesas.descrição_desp, despesas.valor_desp, despesas.datavencimento_desp
FROM Pagamentos JOIN Caixa, Funcionario, Despesas WHERE
...
select * from compra;
select fornecedor.cod_forn, fornecedor.razãosocial_forn, sum(compra.valortotal_comp) as 'Valor total Compra' from compra join fornecedor WHERE 
compra.cod_forn_fk = fornecedor.cod_forn group by compra.cod_forn_fk;

#Tarefa 8
select cliente.nome_cli, avg(venda.valortotal_vend) as 'Preço médio pago', sum(venda.valortotal_vend) 
as 'Soma do preço das vendas feitas' from venda join cliente where venda.cod_cli_fk = cliente.cod_cli group by venda.cod_cli_fk;

#Tarefa 9
select venda.cod_vend, venda.data_vend, venda.valortotal_vend as 'Valor total venda', venda.formpag_vend,
cliente.nome_cli,
sum(itens_venda.quant_itenv) as 'Quantidade total de itens' from venda JOIN cliente, itens_venda WHERE
venda.cod_cli_fk  = cliente.cod_cli AND
itens_venda.cod_vend_fk = venda.cod_vend GROUP by venda.cod_vend;

#Tarefa 10
select count(*) as "Quantidade vendas", funcionario.nome_func from venda join funcionario where venda.cod_func_fk = funcionario.cod_func group by venda.cod_func_fk;


/*create event mostrar
on schedule
 every 1 minute
 starts now()
 comment 'mensagem'
