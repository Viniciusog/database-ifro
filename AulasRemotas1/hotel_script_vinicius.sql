...
DELIMITER $$
CREATE procedure getProdutosComEstoqueMaior(valor int) 
BEGIN 
select * from produto where quantidade_prod > valor;
END
$$ DELIMITER ;

call getProdutosComEstoqueMaior(30);

DELIMITER $$
CREATE procedure inserirProduto(descrição_prod VARCHAR(200), quantidade_prod int, valor_prod float, dataValidade_prod date) 
BEGIN 
INSERT INTO Produto VALUES (null, descrição_prod, quantidade_prod, valor_prod, dataValidade_prod);
SELECT 'produto inserido com sucesso' as Resultado;
END
$$ DELIMITER ;

call inserirProduto('Refrigerante Fanta', 50, 7.99, '2020-09-12');

DELIMITER $$
CREATE procedure getQTDClientes(OUT total int) 
BEGIN 
select count(*) INTO total from cliente;
END
$$ DELIMITER ;

call getQTDClientes(@total);
select @total;

DELIMITER $$
CREATE procedure updatePrecoProdutos(valor int) 
BEGIN 
update produto set valor_prod = valor_prod * ( 1 + (valor_prod / 100)) where cod_prod > 0;
select * from produto;
END
$$ DELIMITER ;

call updatePrecoProdutos(30);

DELIMITER $$
CREATE procedure getValorTotalHospedagem(cod_hospedagem int, dataCheckin_hosp date, dataCheckout_hosp date, valorConsumo_hosp float, valorDiárias_hosp float, valorTotal_hosp float, cod_cli int , cod_func int) 
BEGIN 
update hospedagem set valorTotal_hosp = datediff(dataCheckout_hosp, dataCheckin_hosp) * valorDiárias_hosp + valorConsumo_hosp where cod_hosp = cod_hospedagem;
select * from hospedagem;
END
$$ DELIMITER ;

call getValorTotalHospedagem(1, '2017-03-29', '2017-03-31', 25, 200, 0, 1, 1);